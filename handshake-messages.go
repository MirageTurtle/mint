...
// PSK+mTLS
const (
	HandshakeTypeClientCredential HandshakeType = 65
)

//	struct {
//	    opaque token<1..2^16‑1>;
//	    opaque client_raw_pubkey<1..2^16‑1>;
//	} ClientCredential;
type ClientCredentialBody struct {
	Token           []byte `tls:"head=2,min=1"` // token
	ClientRawPubKey []byte `tls:"head=2,min=1"` // RFC 7250  Raw Public Key
}

func (cc ClientCredentialBody) Type() HandshakeType { return HandshakeTypeClientCredential }

func (cc ClientCredentialBody) Marshal() ([]byte, error) {
	// Just use go-tls-syntax to marshal (for now)
	return syntax.Marshal(cc)
}

func (cc *ClientCredentialBody) Unmarshal(data []byte) (int, error) {
	return syntax.Unmarshal(data, cc)
}
